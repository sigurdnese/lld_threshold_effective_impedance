...
# HOM impedance
Q_2_array = np.array([1,10,200])
f_r2_array = np.linspace(0,1*f_r,500)
f_hom = 1
print(f'hom = {f_hom}')
...
for Q_2 in Q_2_array:
    ...
    files1 = np.array(glob.glob(f'../LHC/results_threshold_bbr_eff_imp/n_m97/lhc_bbr_ImZn0.07_kmax20.0_nrh10.0_phimax1.30_exact_mu2.0_VRF6.0MV_Q_hom{Q_2}_*f_hom1.0_threshold.txt')) 
    files2 = np.array(glob.glob(f'/afs/cern.ch/work/s/snese/cluster/results/results_threshold_bbr_eff_imp/n_m142/lhc_bbr_ImZn0.07_kmax20_nrh10_phimax2.00_exact_mu2.0_VRF6.0MV_Q_hom{Q_2:.1f}_*f_hom1.0_threshold.txt') # Obtained by cluster
    )
    ...
    for files in [files2]:
        ...
        ax.scatter(melody_f_r2/f_r,melody_xi)
        ...
        ax.plot(f_r2_array/f_r,xi_threshold_adaptive,color='black')
    ...
    # ax.legend()
    ax.set_title(f'$\phi_\\mathrm{{max}}={phi_max}$, $Q_2={Q_2}$')
